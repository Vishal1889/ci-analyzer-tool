"""
Resource API downloader for SAP Cloud Integration
Downloads resource metadata from the OData API with parallel processing support
"""
...
class ResourceDownloader(BaseDownloader):
    """Downloader for Integration Flow Resources API with parallel processing"""
    ...
    def __init__(self, *args, parallel_downloads: int = 1, **kwargs):
        ...
        Args:
            parallel_downloads: Number of concurrent IFlow resource downloads (1-10)
        """
    ...
    def download(self) -> Dict[str, Any]:
        """
        Download all resources from all integration flows
        
        Returns:
            Dictionary with download results including:
            - items: List of resource data
            - count: Number of resources downloaded
            - filename: Saved file name
        """
        logger.logger.info("Starting Resource download...")
        ...
        if not iflow_ids:
            logger.logger.warning("No IFlows found. Cannot download Resources.")
            return {
                'items': [],
                'count': 0,
        ...
        # Download Resources with parallel processing
        all_resources = self._download_resources_parallel(iflow_ids)
        ...
        logger.logger.info(f"Resource download completed. Retrieved {len(all_resources)} resources (filtered)")
    ...
    def _download_resources_parallel(self, iflow_ids: List[str]) -> List[Dict[str, Any]]:
        """
        Download Resources for all IFlows using parallel processing
        
        Args:
            iflow_ids: List of IFlow IDs
            
        Returns:
            Consolidated list of all Resources
        """
        all_resources = []
        ...
        if self.parallel_downloads == 1:
            ...
            for i, iflow_id in enumerate(iflow_ids, 1):
                ...
                resources = self._fetch_resources_for_iflow(iflow_id)
            ...
            with ThreadPoolExecutor(max_workers=self.parallel_downloads) as executor:
                # Submit all tasks
                future_to_iflow = {
                    executor.submit(self._fetch_resources_for_iflow, iflow_id): iflow_id 
                ...
                for future in as_completed(future_to_iflow):
                    ...
                    try:
                        ...
                        logger.logger.info(
                            ...
                            f"{iflow_id} ({len(resources)} resources)"
                        )
    ...
    def _fetch_resources_for_iflow(self, iflow_id: str) -> List[Dict[str, Any]]:
        """
        Fetch Resources for a single IFlow and enrich with IflowId and PackageId
        
        Args:
            iflow_id: IFlow ID
            
        Returns:
            List of Resources for this IFlow (enriched with IflowId and PackageId)
        """
        # Build endpoint with proper escaping - use Version='Active'
        endpoint = f"IntegrationDesigntimeArtifacts(Id='{iflow_id}',Version='Active')/Resources"
        ...
        try:
            ...
            logger.logger.debug(f"Fetching Resources for IFlow: {iflow_id}")
            ...
            if 'd' in data and 'results' in data['d']:
                ...
                # Enrich each resource with IflowId and PackageId
                enriched_resources = []
                for resource in resources:
                    resource['IflowId'] = iflow_id
                    resource['PackageId'] = package_id
                    enriched_resources.append(resource)
                
                logger.logger.debug(f"Retrieved {len(enriched_resources)} resources from IFlow {iflow_id}")
            ...
            logger.logger.error(f"Failed to fetch Resources for IFlow {iflow_id}: {e}")
            return []
